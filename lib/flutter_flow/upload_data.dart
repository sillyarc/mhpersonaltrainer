...
import 'video_transcode_stub.dart' if (dart.library.io) 'video_transcode_mobile.dart' as vt;
...
Future<List<SelectedFile>?> selectMedia({
  ...
  // If it's a video, ensure Android-compatible MP4 (H.264/AAC) and cap to 1080p.
  Uint8List processedBytes = mediaBytes;
  if (isVideo && !kIsWeb) {
    try {
      processedBytes = await vt.transcodeToCompatibleMp4(
        pickedMedia!.path,
        mediaBytes,
      );
    } catch (_) {
  ...
  ScaffoldMessenger.of(context)
    ...
    ..showSnackBar(SnackBar(
      content: Text(FFLocalizations.of(context).getText(
        'hbxsjk5b' /* Formato de arquivo inválido */,
      )),
    ));
  return false;
}
