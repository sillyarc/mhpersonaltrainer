...
const buildFeedbackMessage = (params: {
  ...
  if (typeof params.tempoDoTreino === 'number' && params.tempoDoTreino > 0) {
    lines.push(`Tempo do treino: ${formatDuration(params.tempoDoTreino)}`);
  }
  if (params.comment) {
...
const buildFeedbackData = (params: {
  ...
  tempoDoTreino: typeof params.tempoDoTreino === 'number' ? params.tempoDoTreino : undefined,
});
...
const mapFeedback = (id: string, data: any, refPath?: string): FeedbackItem => ({
  ...
  tempoDoTreino: typeof data.tempoDoTreino === 'number' ? data.tempoDoTreino : undefined,
...
export async function submitWorkoutFeedback(payload: {
  ...
  try {
    ...
    const data = removeUndefinedFields({
      ...
      tempoDoTreino: typeof payload.tempoDoTreino === 'number' ? payload.tempoDoTreino : undefined,
    ...
    try {
      ...
      if (personal) {
        ...
        const feedbackData = buildFeedbackData({
          ...
          melhoria: payload.melhoria,
          tempoDoTreino: payload.tempoDoTreino,
        });
        const content = buildFeedbackMessage({
          title: workoutLabel,
          rating: payload.estrela,
