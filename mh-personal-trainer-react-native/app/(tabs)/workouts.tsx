...
export default function WorkoutsScreen() {
  ...
  const listItems: WorkoutListItem[] = [
    ...
    const aName =
      a.kind === 'strength'
        ? a.data.nomeDoTreino
        : (a.data.treinos?.[0] || a.data.treino || 'Treino aeróbico');
    const bName =
      b.kind === 'strength'
        ? b.data.nomeDoTreino
        : (b.data.treinos?.[0] || b.data.treino || 'Treino aeróbico');
    ...
    return (
      ...
      >
        ...
        <Text style={[{ color: colors.secondaryText, marginBottom: spacing.md }, typography.bodyMedium]}>
          {item.obsInstrucao || 'Treino personalizado'}
        </Text>
        <View style={[styles.workoutMeta, { gap: spacing.lg }]}>
          <View style={[styles.metaItem, { gap: spacing.xs }]}>
            ...
            <Text style={[{ color: colors.secondaryText }, typography.labelSmall]}>
              {item.treino?.length || 0} exercícios
            </Text>
          </View>
          <View style={[styles.metaItem, { gap: spacing.xs }]}>
  ...
  const renderAerobicCard = ({ item }: { item: AerobicWorkout }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() =>
          router.push(
            `/workout/aerobico-detail?id=${item.id}&studentId=${targetUserId ?? ''}`
          )
        }
      >
        <View style={[styles.workoutHeader, { marginBottom: spacing.md }]}>
        ...
        <Text style={[{ color: colors.secondaryText, marginBottom: spacing.md }, typography.bodyMedium]}>
          {item.observacoes || item.aquecimento || 'Treino aeróbico'}
        </Text>
        <View style={[styles.workoutMeta, { gap: spacing.lg }]}>
          <View style={[styles.metaItem, { gap: spacing.xs }]}>
            <Ionicons name="list-outline" size={16} color={colors.secondaryText} />
            <Text style={[{ color: colors.secondaryText }, typography.labelSmall]}>
              {treinos.length} atividade(s)
            </Text>
          </View>
          {item.aquecimento ? (
            <View style={[styles.metaItem, { gap: spacing.xs }]}>
        ...
        <View style={[styles.header, { paddingHorizontal: padding, paddingVertical: spacing.md }]}>
          ...
          {isPersonal && (
            <TouchableOpacity
              ...
              onPress={() => router.push(`/workout/create?studentId=${selectedStudentId ?? ''}`)}
            >
        ...
        {isPersonal && (
          <View style={[styles.quickActions, { paddingHorizontal: padding, gap: spacing.sm }]}>
            ...
            >
              ...
              <Text style={[styles.quickActionText, { color: colors.primaryText }]}>Categorias</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.quickActionChip, { backgroundColor: colors.secondaryBackground }]}
            ...
            >
              ...
              <Text style={[styles.quickActionText, { color: colors.primaryText }]}>Aeróbico</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[styles.quickActionChip, { backgroundColor: colors.secondaryBackground }]}
            ...
            >
              ...
              <Text style={[styles.quickActionText, { color: colors.primaryText }]}>Arquivados</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={[styles.emptyContainer, { paddingVertical: spacing['4xl'] }]}>
              ...
              <Text style={[{ color: colors.secondaryText, marginTop: spacing.md, marginBottom: spacing.xl }, typography.bodyLarge]}>
                {loading ? 'Carregando...' : 'Nenhum treino encontrado'}
              </Text>
              {isPersonal && (
                <TouchableOpacity
                  style={[
                  ...
                  onPress={() => router.push(`/workout/create?studentId=${selectedStudentId ?? ''}`)}
                >
                  <Text style={[{ color: colors.info }, typography.titleSmall]}>Criar treino</Text>
                </TouchableOpacity>
              )}
            </View>
          }
