...
export function useAcademyData() {
  ...
  const loadAcademy = useCallback(async () => {
    ...
    if (!academyCodeRaw) {
      setAcademyError('Codigo da academia nao encontrado.');
      setLoadingAcademy(false);
      return;
    }
    ...
    try {
      ...
      const codeVariants = new Set<string | number>();
      if (numeric !== null) codeVariants.add(numeric);
      if (text) codeVariants.add(text);

      if (!codeVariants.size) {
        setAcademyError('Codigo da academia invalido.');
        setPersonals([]);
        setStudents([]);
        return;
      }
  ...
  const loadInvoices = useCallback(async () => {
    if (role !== 'academy' || !academyCodeRaw) {
      setInvoices([]);
    ...
    try {
      ...
      const snapshots = await Promise.all(
        candidates.map((value) => getDocs(query(ref, where('academyCode', '==', value), limit(100))))
      );
