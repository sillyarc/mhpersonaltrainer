...
export async function createExercise(
  exercise: TreinorExercisePayload
...
export async function deleteExercise(exerciseId: string): Promise<QueryResult<void>> {
  try {
    const docRef = doc(db, 'treinors', exerciseId);
    await deleteDoc(docRef);
    return { data: undefined, error: null };
  } catch (error: any) {
    return { data: null, error: error.message };
...
export async function moveExercisesToCollection(
  sourceCollection: string,
  targetCollection: string
): Promise<QueryResult<{ moved: number }>> {
  try {
    const source = sourceCollection.trim();
    const target = targetCollection.trim();

    if (!source) {
      return { data: null, error: 'Colecao de origem invalida.' };
    }

    if (!target) {
      return { data: null, error: 'Colecao de destino invalida.' };
    }

    if (source === target) {
      return { data: null, error: 'Escolha colecoes diferentes para mover os exercicios.' };
    }

    const exercisesRef = collection(db, 'treinors');
    const snapshot = await getDocs(exercisesRef);
    const docsToMove = snapshot.docs.filter((docSnap) => {
      const currentCollection = String(docSnap.data().colecao || 'geral').trim();
      return currentCollection === source;
    });

    if (!docsToMove.length) {
      return { data: { moved: 0 }, error: null };
    }

    const updatedAt = Timestamp.now();
    for (let i = 0; i < docsToMove.length; i += BATCH_LIMIT) {
      const chunk = docsToMove.slice(i, i + BATCH_LIMIT);
    ...
    return { data: { moved: docsToMove.length }, error: null };
  } catch (error: any) {
    return { data: null, error: error.message };
  }
}

export async function deleteCollectionExercises(
  collectionName: string
): Promise<QueryResult<{ deleted: number }>> {
  try {
    const source = collectionName.trim();

    if (!source) {
      return { data: null, error: 'Colecao invalida.' };
    }
    ...
    return { data: { deleted: docsToDelete.length }, error: null };
  } catch (error: any) {
    return { data: null, error: error.message };
  }
}
...
export async function fetchUserWorkouts(
  ...
  try {
    ...
    const q = query(workoutsRef, orderBy('nomeDoTreino', 'desc'));
    const snapshot = await getDocs(q);
...
export async function fetchExerciseByName(name: string): Promise<QueryResult<Exercise>> {
  try {
    const exercisesRef = collection(db, 'treinors');
    const runQuery = async (field: string) => {
      const q = query(exercisesRef, where(field, '==', name), limit(1));
      return getDocs(q);
    };

    let snapshot = await runQuery('treinosNoLIst');
    if (snapshot.empty) {
      snapshot = await runQuery('nomeDoTreino');
    }

    if (snapshot.empty) {
      return { data: null, error: 'Exercicio nao encontrado' };
    }
