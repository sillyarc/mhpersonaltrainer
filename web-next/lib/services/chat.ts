...
export function listenToSupportConversations(
  callback: (items: Conversation[]) => void
) {
  const db = getFirebaseDb();
  const conversationsRef = collection(db, 'conversations');
  const q = query(conversationsRef, where('participants', 'array-contains', 'support'));
  return onSnapshot(
    q,
    (snapshot) => {
      const items = snapshot.docs.map((docSnap) => mapConversation(docSnap.id, docSnap.data()));
    ...
    (error) => {
      console.error('Erro ao escutar conversas de suporte:', error);
      callback([]);
    }
  );
}
