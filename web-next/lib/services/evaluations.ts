...
export async function createPosturalEvaluation(
  ...
  try {
    ...
    const evalRef = collection(db, 'users', evaluation.userId, collectionName);
    const observacoes = evaluation.analise?.observacoes || null;
    const hasLateral = Boolean(
      evaluation.fotosPostura?.lateralDireita || evaluation.fotosPostura?.lateralEsquerda
    );
    const hasPosterior = Boolean(evaluation.fotosPostura?.posterior);
    const hasFrontal = Boolean(evaluation.fotosPostura?.anterior);
    const docRef = await addDoc(evalRef, {
      uid: evaluation.userId,
      ...
      obsFotoFrontal: hasFrontal ? observacoes : null,
      obsFotoPosterior: hasPosterior ? observacoes : null,
      obsFotoLateral: hasLateral ? observacoes : null,
      createdAt: Timestamp.now(),
      updatedAt: Timestamp.now(),
    });
