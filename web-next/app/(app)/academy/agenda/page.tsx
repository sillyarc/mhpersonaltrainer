...
export default function AcademyAgendaPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setLoading(true);
      setError('');
      const academyStudentMap = new Map(academyStudents.map((student) => [student.id, student]));
      const studentIds = Array.from(academyStudentMap.keys());
      ...
      const appointmentItems: AcademyAgendaItem[] = (appointmentsResult.data || [])
        .filter((appointment) => academyStudentMap.has(appointment.alunoId))
        .map((appointment) => {
          const linkedStudent = academyStudentMap.get(appointment.alunoId);
          return {
            ...appointment,
            alunoNome: linkedStudent?.name || appointment.alunoNome,
            personalNome: linkedStudent?.personalName || appointment.personalNome,
            source: 'appointment',
